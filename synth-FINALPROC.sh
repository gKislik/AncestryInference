...
while read line; do
	...
	#qsub proc1-pca-redone-fst.sh ${pref} #if starting from fastq
	#bash proc1-pca-mergefirst-synth.sh ${pref}
	#qsub proc1-pca-synthvcf.sh ${pref} #if starting from vcf
	#qsub proc1-pca-synth-final.sh ${pref}
	qsub newapproach-synth-final.sh ${pref}
done < $list
