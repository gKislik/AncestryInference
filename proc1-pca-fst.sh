...
seed=12345
#ref=/u/project/pellegrini/gkislik/all_geno/final/LABR-POOD-CKSP/SAFETY/ref-SRR8614074
#ref=/u/project/pellegrini/gkislik/all_geno/final/LABR-POOD-CKSP/SAFETY/ref-SRR8614076
...
### for some reason using -R with the two column version of pos_test_var.txt doesn't capture all the SNPs
### figuring this out would save a lot of time, but this method still works 
...
awk '$2="chr"$1"_"$4' ${inp}_test.bim > ${inp}_test_temp.txt
mv ${inp}_test_temp.txt ${inp}_test.bim

plink -bfile ${inp}_test -chr-set 38 -allow-extra-chr -make-bed -out ${inp}_test2 -extract finSNPs.txt #pos_test_var.txt #<-pos_test_var.txt is for only FST filtering, before removing weird samples
plink -bfile $ref -bmerge ${inp}_test2 -make-bed -chr-set 38 -allow-extra-chr -out ${inp}_test_merged -indiv-sort 0
...
plink -bfile ${inp}_nm -bmerge ${inp}_ref -chr-set 38 -allow-extra-chr -make-bed -out ${inp}_merge -indiv-sort 0
...
#plink -bfile ${inp}_prefin -within sixclust.txt -fst -chr-set 38 -allow-extra-chr -out ${inp}_fst
#awk '$5>0.03 && $5 != "nan" {print $2}' ${inp}_fst.fst > ${inp}_var.txt
...
#plink -bfile ${inp}_test_merged -chr-set 38 -allow-extra-chr -make-bed -out ${inp}_fin -extract ${inp}_var.txt #${inp}_ufinSNPs.txt
